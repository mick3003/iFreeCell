...
AFreecellGameBoard* CreateAFreecellGameBoard(char* GameName, bool Display10AsT)
{
	...
	case BOARD_ID_BAKERS_DOZEN:
		return new PysolBakersDozenGameBoard(Display10AsT);
	case BOARD_ID_BELEAGUERED_CASTLE:
		return new AisleRiotBeleagueredCastleGameBoard(Display10AsT);
	case BOARD_ID_CITADEL:
		return new PysolCitadelGameBoard(Display10AsT);
	case BOARD_ID_DIE_SCHLANGE:
		return new PysolDieSchlangeGameBoard(Display10AsT);
	case BOARD_ID_DER_KATZENSCHWANZ:
		return new PysolDerKatzenschwantzGameBoard(Display10AsT);
	case BOARD_ID_EIGHT_OFF:
		return new AisleRiotEightOffGameBoard(Display10AsT);
	case BOARD_ID_FAN:
		return new PysolFanGameBoard(Display10AsT);
	case BOARD_ID_FORECELL:
		return new PysolForecellGameBoard(Display10AsT);
	...
	case BOARD_ID_STREETS_AND_ALLEYS:
		return new AisleRiotFreecellGameBoard(Display10AsT);
	case BOARD_ID_GNOME_FREECELL:
		return new GnomeFreecellGameBoard(Display10AsT);
	case BOARD_ID_GYPSY:
		return new PysolGypsyGameBoard(Display10AsT);
	case BOARD_ID_KLONDIKE:
		return new AisleRiotKlondikeGameBoard(Display10AsT);
	case BOARD_ID_MICROSOFT_FREECELL:
		return new MicrosoftFreecellGameBoard(Display10AsT);
	case BOARD_ID_PYSOL_BELEAGUERED_CASTLE:
		return new PysolBeleagueredCastleGameBoard(Display10AsT);
	case BOARD_ID_PYSOL_EIGHT_OFF:
		return new PysolEightOffGameBoard(Display10AsT);
	...
	case BOARD_ID_PYSOL_STREETS_AND_ALLEYS:
		return new PysolFreecellGameBoard(Display10AsT);
	case BOARD_ID_PYSOL_KLONDIKE:
		return new PysolKlondikeGameBoard(Display10AsT);
	case BOARD_ID_PYSOL_RELAXED_SEAHAVEN_TOWERS:
	case BOARD_ID_PYSOL_SEAHAVEN_TOWERS:
		return new PysolSeahavenTowersGameBoard(Display10AsT);
	case BOARD_ID_SEAHAVEN_TOWERS:
	case BOARD_ID_RELAXED_SEAHAVEN_TOWERS:
		return new AisleRiotSeahavenTowersGameBoard(Display10AsT);
	case BOARD_ID_SIMPLE_SIMON:
		return new PysolSimpleSimonGameBoard(Display10AsT);
	case BOARD_ID_SMALL_HARP:
		return new PysolSmallHarpGameBoard(Display10AsT);
	case BOARD_ID_WHITEHEAD:
		return new PysolWhiteheadGameBoard(Display10AsT);
	case BOARD_ID_YUKON:
		return new PysolYukonGameBoard(Display10AsT);
	case BOARD_ID_NONE:
	default:
		return NULL;
	}
