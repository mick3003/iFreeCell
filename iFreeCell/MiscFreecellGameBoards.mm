...
int MicrosoftRandom(int* Seed)
{
    *Seed = ((*Seed) * 214013 + 2531011);
    return ((*Seed) >> 16) & 0x7fff; 
}

MicrosoftFreecellGameBoard::MicrosoftFreecellGameBoard(bool Display10AsT) : AFreecellGameBoard(Display10AsT)
{
}
...
void MicrosoftFreecellGameBoard::Shuffle(int Seed)
{
	...
	{
		j = MicrosoftRandom((int*)&Seed) % wLeft;
        // NSLog(@"seed = %d", (int) Seed);
		m_Deck[i] = TempDeck[j];
...
void GnomeFreecellGameBoard::Shuffle(int Seed)
{
	...
	for (i = 0; i < GetNumberOfCardsInDeck(); i++)
	{
		Index = MicrosoftRandom((int*)&Seed) % (i+1);
		for (j = i; j > Index; j--)
			m_Deck[j] = m_Deck[j-1];
		m_Deck[Index] = 51 - i;
	}
...
void AisleRiotGameBoard::Shuffle(int Seed)
{
	...
	for(i=0 ; i<GetNumberOfCardsInDeck() ; i++)
	{
		j = i + (MicrosoftRandom((int*)&Seed)%(GetNumberOfCardsInDeck()-i));
		TempCard = m_Deck[j];
		m_Deck[j] = m_Deck[i];
		m_Deck[i] = TempCard;
	}
